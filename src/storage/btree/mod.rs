...
impl BPlusTree {
    fn split_leaf_and_maybe_root(&mut self, leaf_id: NodeId) {
        let (separator, new_leaf_id) = self.split_leaf(leaf_id);
        self.assert_invariants();
        ...
        // Minimal implementation: only root splits supported
        panic!("Internal node splitting not implemented (by design)");
    }
}
    ...
    fn rebalance_leaf(&mut self, parent: NodeId, idx: usize) {
        ...
        if let (Some(lidx), Some(sep_idx)) = (left_idx, sep_left) {
            ...
            if self.node_key_count(left_id) > min_keys {
                ...
                let new_sep = leaf.keys[0].clone(); // capture while borrowed
                // leaf + left borrows END here
        ...
        if let (Some(ridx), Some(sep_idx)) = (right_idx, sep_right) {
            ...
            if self.node_key_count(right_id) > min_keys {
                ...
                let new_sep = right.keys[0].clone(); // capture first
                // leaf + right borrows END here
